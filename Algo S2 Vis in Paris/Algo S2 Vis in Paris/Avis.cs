...
namespace Algo_S2_Vis_in_Paris
{
    internal class Avis
    {
        ...
        public void SupprimerAvis(Avis test)
        {
            ConnexionSQL a = new ConnexionSQL();
            string requetetable = $"DELETE FROM AVIS WHERE ID_AVIS='{test.IdAvis}';";
            try
            {
                MySqlCommand command = a.MaConnexion.CreateCommand();
                command.CommandText = requetetable;
            ...
            {
                Console.WriteLine("Erreur lors de la suppression de l'avis : " + e.Message);
            }
            finally
            {
                a.MaConnexion.Close();
